...
let has_complementary_literals formulas =
...
let has_formula_and_negation formulas =
...
let can_close_node formulas = has_complementary_literals formulas || has_formula_and_negation formulas


let all_formulas_are_literals formulas =
...
let extract_valuation_from_branch formulas =
...
let create_initial_node formula = {
	formulas = [formula];
	marked = false;
	node_type = Literal
}
...
let apply_alpha_rule node alpha_formula =
...
let apply_beta_rule node beta_formula =
...
let rec find_decomposable_formula formulas =
	match formulas with
	| [] -> None
	| h :: t ->
...
let semantic_tableaux_propositional formula =
	let rec build_tableaux nodes =
		match nodes with
		| [] -> TableauUnknown
		| node :: rest ->
			if node.marked then
...
let analytic_apply_alpha_rule node alpha_formula =
	let components = get_alpha_components alpha_formula in
	{ node with formulas = components }  (* Don't keep other formulas *)


let analytic_apply_beta_rule node beta_formula =
	let components = get_beta_components beta_formula in
	match components with
	| [comp1; comp2] ->
		[
...
let analytic_tableaux_propositional formula =
	let rec build_analytic_tableaux branch_nodes =
...
let complete_semantic_tableaux_propositional formula =
	semantic_tableaux_propositional formula


(* Top-level proving functions *)

let prove_by_tableaux formula =
...
let is_unsatisfiable_tableaux formula =
